    ...
    public class ClientProfileSeed
    {
        public static void Seeder(AccountsContext context){
            ...
            clientProfiles.Add(new ClientProfile{
                ...
                ProfileId =  context.Profiles.AsNoTracking().FirstOrDefault(w => w.App.Slug == "team-accounts-api" && w.Slug == "login").Id
            });

            clientProfiles.Add(new ClientProfile{
                ClientId = new Guid("e7f8a9b0-1c2d-3e4f-5a6b-7c8d9e0f1a2b"),
                ProfileId =  context.Profiles.AsNoTracking().FirstOrDefault(w => w.App.Slug == "team-accounts-api" && w.Slug == "admin").Id
            });


            clientProfiles.Add(new ClientProfile{
                ClientId = new Guid("9bc91fc4-e79a-4f68-9b9a-693ddf61a7e3"),
                ProfileId =  context.Profiles.AsNoTracking().FirstOrDefault(w => w.App.Slug == "team-accounts-api" && w.Slug == "public-token").Id
            });
